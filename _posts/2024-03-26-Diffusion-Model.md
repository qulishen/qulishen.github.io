...
$x_t=\sqrt{\overline{\alpha_t}} x_0+\sqrt{1-\overline{\alpha_t}} \epsilon$ 和 $x_t=\sqrt{1-\beta_t} x_{t-1}+\sqrt{\beta_t} \epsilon $ 可以得到:
...
# 4. 第四节DDIM
DDPM的缺陷在于,推理速度过慢,因为其本身是一个马尔科夫链的过程,无法进行跳跃预测,即无法通过$x_t$直接去预测$x_{t-2}$,于是就有了 DDIM 的出现. 无需重新训练 DDPM,只需要对采样器进行修改即可.
